...
#	z_max=1/((1/z_now)-(E_kin_mof+E_th_plm+E_kin_o)/(G*M_dyn*M_of))
#	z_max=1/((1/z_now)-(0.5*M_of*v_now**2+(E_th_plm+E_kin_o))/(G*M_dyn*M_of))
#	z_max_kpc=z_max/kpc
#	g_z_now=G*M_dyn/z_now**2
#	g_z_max=G*M_dyn/z_max**2
#	g_r=0.5*(g_z_now+g_z_max)
	v_rot=v_rot_kms*1.0e5
	r_rot=r_rot_kpc*kpc
	g_r=2*v_rot**2/r_rot
	...
	z_max=-GM/Phi2
	z_max_kpc=z_max/kpc
	Phi=2*math.pi*v_rot**2/r_rot*(z_now-math.sqrt(z_now**2+r_rot**2))
	v_esc=math.sqrt(2*abs(Phi))
	...
	g_r_now_Kuzmin=GM/z_now
	g_r_max_Kuzmin=GM/z_max
	g_r_100pc_Kuzmin=GM/(0.1*kpc)
	g_r_10pc_Kuzmin=GM/(10*pc)
	g_r_1pc_Kuzmin=GM/pc
#	v_max=math.sqrt(abs(v_go**2+2*(E_th_plm+E_kin_o)/M_of-2*G*M_dyn/z_now))
#	v_max=math.sqrt(2*G*M_dyn/z_max)
#	v_max_kms=v_max/1.0e5
#	a=2*G*M_dyn/r1+v1**2
#	b=-2*G*M_dyn
	t_top=v_go/g_r
	t_top_myr=t_top/Myr
	t_return=v_go/g_r+math.sqrt((v_go/g_r)**2+2*z_max/g_r)
	t_return_=v_go/g_r+math.sqrt((v_go/g_r)**2-2*z_max/g_r)
...
#	r_max_kpc=r_max/kpc
#	print '%.2f' %t1_myr, "[Myr]", '%.2f' %t_top_myr, "[Myr]",  '%.2f' %t_myr, "[Myr]", type
	print type, "z_max",'%.2f' %z_max_kpc, "[kpc] | t_top =", '%.2f' %t_top_myr, "[Myr] | g =", '%.2e' %g_r, "[cm s-2]"
...
#	print "     g =", '%.2e' %g_r, "(",'%.2e' %g_z_now, "--", '%.2e' %g_z_max,")"
	print "     v_now =", '%.2f' %v1_kms, "[km/s] | v_go =",'%.2f' %v_go_kms, "[km/s]"
	print "     v_esc =", '%.2f' %v_esc_kms, "[km/s]"
...
#	print "     t_return_ =", '%.2f' %t_return_myr_, "[Myr]"
	...
	print "     z_max_Kuzmin =", '%.2f' %z_max_kpc_Kuzmin, "[kpc]"
	print "     g_r_now_Kuzmin =", '%.2e' %g_r_now_Kuzmin, "[cm/s^2]"
	print "     g_r_max_Kuzmin =", '%.2e' %g_r_max_Kuzmin, "[cm/s^2]"
	print "     g_r_100pc_Kuzmin =", '%.2e' %g_r_100pc_Kuzmin, "[cm/s^2]"
	print "     g_r_10pc_Kuzmin =", '%.2e' %g_r_10pc_Kuzmin, "[cm/s^2]"
	print "     g_r_1pc_Kuzmin =", '%.2e' %g_r_1pc_Kuzmin, "[cm/s^2]"
	return t_top_myr

print "---------------------------"
tff_max(M_dyn_msun_n2146,M_mof_msun_n2146,E_kin_mof_n2146,E_th_plm_n2146,N_ostar_n2146,effi_ostar,z2_mof_kpc_n2146,v_mof_kms_n2146,v_rot_HI_kms_n2146,r_rot_HI_kpc_n2146, "ngc2146 (M) |")
tff_max(M_dyn_msun_n2146,M_mof_msun_n2146,E_kin_mof_n2146,1.3e55,N_ostar_n2146,0.2,z2_mof_kpc_n2146,v_mof_kms_n2146,v_rot_HI_kms_n2146,r_rot_HI_kpc_n2146, "ngc2146 (U) |")
tff_max(M_dyn_msun_n2146,M_mof_msun_n2146,E_kin_mof_n2146,0.4e55,N_ostar_n2146,0.1,z2_mof_kpc_n2146,v_mof_kms_n2146,v_rot_HI_kms_n2146,r_rot_HI_kpc_n2146, "ngc2146 (L) |")
print "---------------------------"
tff_max(M_dyn_msun_n3628,M_mof_msun_n3628,E_kin_mof_n3628,E_th_plm_n3628,N_ostar_n3628,effi_ostar,z2_mof_kpc_n3628,v_mof_kms_n3628, v_rot_HI_kms_n3628,r_rot_HI_kpc_n3628,"ngc3628 (M) |")
tff_max(M_dyn_msun_n3628,M_mof_msun_n3628,2.8e54,2.3e53,N_ostar_n3628,0.2,z2_mof_kpc_n3628,v_mof_kms_n3628, v_rot_HI_kms_n3628,r_rot_HI_kpc_n3628,"ngc3628 (U) |")
tff_max(M_dyn_msun_n3628,M_mof_msun_n3628,1.8e54,0.7e53,N_ostar_n3628,0.1,z2_mof_kpc_n3628,v_mof_kms_n3628, v_rot_HI_kms_n3628,r_rot_HI_kpc_n3628,"ngc3628 (L) |")
print "---------------------------"
#  --------------------------------------------	#
