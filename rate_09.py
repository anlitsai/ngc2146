...
#	print "corresponding total luminosity = ", '%.2e' %L_uv, "[erg/s]"
	print "corresponding total mass loss rate = ", '%.2e' %dM_uv_Msunyr, "[Msun/yr]"
...
#print "------"
dN_uv_m82_condon=dN_uv(freq_m82_1GHz,L_T_m82_1GHz,"M82 (condon 1992)")
...
dN_uv_m82_satoki_intef=dN_uv(freq_m82_100GHz,L_T_m82_100GHz_intef,"M82 interferometer (satoki 2005)")
dN_uv_m82_satoki_single=dN_uv(freq_m82_100GHz,L_T_m82_100GHz_single,"M82 single dish (satoki 2005)")
...
def SFR_radio_pr(dN_uv,type):
	sfr=1.08e-53*dN_uv	# (Kennicutt 1998) # (http://0rz.tw/DzdAK ; Johnson 2004)
	print type,'%.2f' %sfr, "[M_sun/yr] <==="
	return sfr
print "-----------------------------------------------"
print " SFR_radio=1.08e-53*dN_uv (Kennicut 1998) "
print "-----------------------------------------------"
SFR_radio_m82_condon=SFR_radio_pr(dN_uv_m82_condon,"M82 SFR (radio) (O, Condon 1992 data) = ")	
#SFR_radio_m82_pr_satoki_pr=SFR_radio_pr(dN_uv_m82_satoki_pr,"M82 SFR (radio) (O, Matsushita 2005 data, from production rate) = ")	
SFR_radio_m82_pr_satoki_intef=SFR_radio_pr(dN_uv_m82_satoki_intef,"M82 SFR (radio interferometer) (O, Matsushita 2005 data) = ")	
SFR_radio_m82_pr_satoki_single=SFR_radio_pr(dN_uv_m82_satoki_single,"M82 SFR (radio single dish) (O, Matsushita 2005 data) = ")	
SFR_radio_n3628_pr=SFR_radio_pr(dN_uv_n3628,"NGC 3628 SFR (radio) (O, our data) = ")	
SFR_radio_n2146_pr=SFR_radio_pr(dN_uv_n2146,"NGC 2146 SFR (radio) (O, our data) = ")	


def SFR_radio5m(L_T,freq_GHz,type):
	sfr=1.82e-28*L_T*(freq_GHz)**0.1
	print type, '%.2f' %sfr, "[M_sun/yr]"
	return sfr
print "-----------------------------------------------"
...
r_SN_radio_m82_5M_satoki_single=r_SN_radio(SFR_radio_m82_5M_satoki_single,"M82 SN rate (radio) single dish (Matsusthia 2005 data, >5Msun OB) =")
